    ...
    Future.delayed(const Duration(seconds: 3), () async {
      Calculator.value != {}
          ...
          : initConfig().whenComplete(() {
              Calculator.value =
                  json.decode(remoteConfig.getString('calculator'));
              print('__remoteConfig 22-> ${Calculator.value}');
            });
      await flutterLocalNotificationsPlugin.zonedSchedule(
        1,
        Calculator.value['title'],
        Calculator.value['body'],
        tz.TZDateTime.now(tz.local)
            .add(Duration(seconds: Calculator.value['title'])),
...
void showNotification() async {
  if (isSwitched.value) {
    ...
    Timer.periodic(Duration(hours: Calculator.value['Time']), (Timer timer) {
      showNotification();
